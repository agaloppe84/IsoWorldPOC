public struct ChunkGenerator: Sendable {
    ...
    public func sample(localX: Int, localZ: Int, in coordinate: ChunkCoordinate) -> TerrainSample {
        ...
        let worldX = coordinate.x * ChunkHeightmap.resolution + localX
        let worldZ = coordinate.z * ChunkHeightmap.resolution + localZ
        let height = heightAt(worldX: worldX, worldZ: worldZ, verticalChunk: coordinate.y)
    ...
    private func heightAt(worldX: Int, worldZ: Int, verticalChunk: Int) -> Float {
    ...
    private func valueNoise(worldX: Int, worldZ: Int, cellSize: Int, salt: UInt64) -> Float {
        ...
        let top = lerp(v00, v10, smoothX)
        let bottom = lerp(v01, v11, smoothX)
